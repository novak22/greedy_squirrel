...
function validateGameConfig() {
    ...
    if (
        !isNonEmptyArray(GAME_CONFIG.betOptions) ||
        !GAME_CONFIG.betOptions.every(isPositiveInteger)
    ) {
    ...
    if (
        !(
            typeof GAME_CONFIG.maxBetIncrementPercent === 'number' &&
            GAME_CONFIG.maxBetIncrementPercent > 0 &&
            GAME_CONFIG.maxBetIncrementPercent <= 1
        )
    ) {
    ...
    if (
        !(
            typeof GAME_CONFIG.targetRTP === 'number' &&
            GAME_CONFIG.targetRTP > 0 &&
            GAME_CONFIG.targetRTP <= 100
        )
    ) {
    ...
    if (
        !GAME_CONFIG.winThresholds ||
        !isPositiveNumber(GAME_CONFIG.winThresholds.big) ||
        !isPositiveNumber(GAME_CONFIG.winThresholds.mega)
    ) {
        errors.push('GAME_CONFIG.winThresholds must include positive numbers for big and mega');
    }

    if (
        !isNonEmptyArray(GAME_CONFIG.paylines) ||
        GAME_CONFIG.paylines.some(
            (line) =>
                !Array.isArray(line) ||
                line.length !== GAME_CONFIG.reelCount ||
                line.some(
                    (index) =>
                        !Number.isInteger(index) || index < 0 || index >= GAME_CONFIG.rowCount
                )
        )
    ) {
        errors.push(
            'GAME_CONFIG.paylines must be arrays matching reelCount with row indices within rowCount'
        );
    }

    if (
        !isNonEmptyArray(GAME_CONFIG.spinDurations) ||
        GAME_CONFIG.spinDurations.length !== GAME_CONFIG.reelCount ||
        GAME_CONFIG.spinDurations.some((duration) => !isPositiveInteger(duration))
    ) {
    ...
    anticipationFields.forEach((field) => {
        if (
            !(
                typeof anticipation[field] === 'number' &&
                anticipation[field] >= 0 &&
                anticipation[field] <= 1
            )
        ) {
...
function validateFeaturesConfig() {
    ...
    if (
        !isNonEmptyArray(freeSpins.multipliers) ||
        freeSpins.multipliers.some((multiplier) => !isPositiveNumber(multiplier))
    ) {
    ...
    if (
        !isPositiveInteger(pickGame.minPicks) ||
        !isPositiveInteger(pickGame.maxPicks) ||
        pickGame.minPicks > pickGame.maxPicks
    ) {
        errors.push(
            'FEATURES_CONFIG.bonusGame.pickGame minPicks/maxPicks must be positive integers with minPicks <= maxPicks'
        );
    }
    ...
    if (!isPositiveInteger(gamble.maxWinAmount) || !isPositiveInteger(gamble.maxAttempts)) {
        errors.push(
            'FEATURES_CONFIG.gamble.maxWinAmount and maxAttempts must be positive integers'
        );
    }
    ...
    if (
        cascade.enabled &&
        (!isPositiveInteger(cascade.maxIterations) || !isNonEmptyArray(cascade.multipliers))
    ) {
        errors.push(
            'FEATURES_CONFIG.cascade requires valid maxIterations and multipliers when enabled'
        );
    }
...
function validateProgressionConfig() {
    ...
    if (typeof levels.xpPerLevel !== 'function' || !isPositiveInteger(levels.xpPerLevel(1))) {
        errors.push(
            'PROGRESSION_CONFIG.levels.xpPerLevel must be a function returning positive integers'
        );
    }
    ...
    if (!Array.isArray(PROGRESSION_CONFIG.achievements)) {
        ...
        PROGRESSION_CONFIG.achievements.some(
            (achievement) => !(achievement.id && typeof achievement.check === 'function')
        )
    ) {
...
function validateSymbolsConfig() {
    ...
    Object.entries(SYMBOLS).forEach(([key, symbol]) => {
        ...
        if (
            symbol.allowedReels &&
            (!Array.isArray(symbol.allowedReels) ||
                symbol.allowedReels.some((index) => !Number.isInteger(index) || index < 0))
        ) {
