...
export class PaylineEvaluator {
    /**
     * Create a PaylineEvaluator with game configuration
     * @param {Object} config - Game configuration
     * @param {Object} config.symbols - Symbol configuration (SYMBOLS object)
     * @param {Object} config.symbolHelpers - Symbol helper functions { getSymbolByEmoji }
     * @param {Array} config.paylines - Array of payline patterns
     * @param {number} config.reelCount - Number of reels
     * @param {Object} [config.metrics] - Optional metrics instance
     */
    constructor(config) {
        this.symbols = config.symbols;
        this.getSymbolByEmoji = config.symbolHelpers?.getSymbolByEmoji || (() => null);
    ...
    evaluateWins(result, betAmount) {
        ...
        const evalTimer = this.metrics?.startTimer?.('spin.evaluation', { betAmount });
        let totalWin = 0;
     ...
     */
    checkPaylineWins(result, betAmount) {
        let totalWin = 0;
        const winningPositions = new Set();
        const winningLines = [];
        const details = [];
     ...
     */
    evaluatePayline(symbols, positions, betAmount) {
        const wildEmoji = this.symbols.WILD.emoji;
        const scatterEmoji = this.symbols.SCATTER.emoji;
        const bonusEmoji = this.symbols.BONUS.emoji;
        ...
        if (matchCount >= 3) {
            const symbolConfig = this.getSymbolByEmoji(baseSymbol);
     ...
     */
    checkScatterWins(result, betAmount) {
        const scatterEmoji = this.symbols.SCATTER.emoji;
        ...
        if (scatterCount >= 3) {
            ...
            const payout = this.symbols.SCATTER.payouts[payoutKey];
